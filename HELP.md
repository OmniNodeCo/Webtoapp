...
Below you'll find a list of build commands contributed by the Webtoapp community. They are here as examples, to help you wrap "complicated" apps that need a bit of elbow grease to work. We need your help to enrich it, as long as you follow these two guidelines:

1. Only add sites that require something special! No need to document here that `simplesite.com` works with a simple `webtoapp simplesite.com` 🙂.
...
Some sites like [HBO Max](https://github.com/OmniNodeCo/Webtoapp/issues/1153) and [Udemy](https://github.com/OmniNodeCo/Webtoapp/issues/1147) host videos using [DRM](https://en.wikipedia.org/wiki/Digital_rights_management).

For those, try passing the [`--widevine`](DOCS.md#widevine) option.
...
Try deleting your app's cache, found at `<your_app_name_lower_case>-webtoapp-<random_id>` in your OS’s "App Data" directory (Linux: `$XDG_CONFIG_HOME` or `~/.config` , MacOS: `~/Library/Application Support/` , Windows: `%APPDATA%` or `C:\Users\yourprofile\AppData\Roaming`)
...
```sh
webtoapp 'https://open.google.com/'
  --inject window.js
```

Note: [Inject](https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#inject)
...
```sh
webtoapp 'https://docs.google.com/spreadsheets' \
  --user-agent firefox
```
...
```sh
webtoapp 'https://outlook.office.com/mail'
  --internal-urls '.*?(outlook.live.com|outlook.office365.com).*?'
  --file-download-options '{"saveAs": true}'
  --browserwindow-options '{"webPreferences": { "webviewTag": true, "nodeIntegration": false } }'
```
...
```sh
webtoapp 'https://www.udemy.com/'
  --internal-urls '.*?udemy.*?'
  --file-download-options '{"saveAs": true}'
  --widevine
```

Note: most videos will work, but to play some DRMed videos you must pass `--widevine` AND [sign the app](https://github.com/OmniNodeCo/Webtoapp/issues/1147#issuecomment-828750362).

### HBO Max

```sh
webtoapp 'https://play.hbomax.com/'
  --widevine
  --enable-es3-apis
&& python -m castlabs_evs.vmp sign-pkg 'name_of_the_generated_hbo_app'
```

Note: as for Udemy, `--widevine` + [app signing](https://github.com/OmniNodeCo/Webtoapp/issues/1147#issuecomment-828750362) is necessary.

### WhatsApp

```sh
webtoapp 'https://web.whatsapp.com/'
  --inject whatsapp.js
```

With this `--inject` in `whatsapp.js` (and maybe more, see [#1112](https://github.com/OmniNodeCo/Webtoapp/issues/1112)):
...
```sh
webtoapp https://web.whatsapp.com --name 'WhatsApp Business' --counter true --darwin-dark-mode-support true --title-bar-style hidden --inject whatsappmacos.css
```
...
```sh
webtoapp 'https://open.spotify.com/'
  --widevine
  --inject spotify.js
  --inject spotify.css
```
...
-   You might have to pass `--user-agent firefox` to circumvent Spotify's detection that your browser isn't a real Chrome. But [maybe not](https://github.com/OmniNodeCo/Webtoapp/issues/1195#issuecomment-855003776).
-   [Inject](https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#inject) the following javascript as `spotify.js` to prevent "Unsupported Browser" messages.
...
-   It is also required to [sign the app](https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#widevine), or many songs will not play.
-   To hide all download links (as if you were in the actual app), [inject](https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#inject) the following CSS as `spotify.css`:
...
You can use Notion pages with Webtoapp without much hassle, but Notion itself does not present an easy way to use HTML buttons. As such, if you want to use Notion Pages as a quick way to make dashboards and interactive panels, you will be restricted to only plain links and standard components.

With Webtoapp you can now extend Notion's functionality and possibilities by adding HTML buttons that can call other javascript functions, since it enables you to inject custom Javascript and CSS.

```sh
webtoapp 'YOUR_NOTION_PAGE_SHARE_URL'
  --inject notion.js
  --inject notion.css
```
...
-   You can inject the notion.js and notion.css files by copying them to the resources/app/inject folder of your webtoapp app.
...
```sh
webtoapp https://teams.microsoft.com --name 'Microsoft Teams' --counter true --darwin-dark-mode-support true --title-bar-style hidden --internal-urls "(.*)" --inject teamsapp.css
```
