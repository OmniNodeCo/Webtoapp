...
import { DEFAULT_PAKE_OPTIONS } from '../../cli/defaults';
import type { PakeAppOptions } from '../../cli/types';

const tempDirs: string[] = [];

async function makeTempDir() {
  const tempDir = await fsExtra.mkdtemp(path.join(os.tmpdir(), 'webtoapp-icon-'));
...
async function importLinuxIconModule() {
  ...
  return await import('../../cli/options/icon');
}

function makeOptions(icon: string): PakeAppOptions {
  return {
    ...DEFAULT_PAKE_OPTIONS,
    identifier: 'com.webtoapp.test',
    name: 'Linux App',
    icon,
  };
}
