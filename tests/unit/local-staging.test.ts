...
const baseDir = path.join(os.tmpdir(), `webtoapp-staging-${process.pid}`);
...
vi.mock('@/utils/dir', async () => {
  ...
  const base = nodePath.join(nodeOs.tmpdir(), `webtoapp-staging-${process.pid}`);
  return {
    npmDirectory: nodePath.join(base, 'pkg'),
    tauriConfigDirectory: nodePath.join(base, 'pkg', 'src-tauri', '.webtoapp'),
  };
});
