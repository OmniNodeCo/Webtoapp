...
import schema from '../../schema/webtoapp.schema.json';
...
describe('webtoapp.schema.json stays in sync with the CLI surface', () => {
...
describe('loadConfigFile', () => {
  ...
  beforeAll(async () => {
    tmpDir = await fsExtra.mkdtemp(path.join(os.tmpdir(), 'webtoapp-config-'));
  });
  ...
  it('loads url and camelCase options', async () => {
    const configPath = await writeConfig({
      $schema: 'https://example.com/webtoapp.schema.json',
