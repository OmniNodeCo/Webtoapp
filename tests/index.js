...
class PakeTestRunner {
  ...
  async runPakeCliTests() {
    ...
    await this.runTest(
      "webtoapp-cli Package Installation",
      async () => {
        try {
          execSync("pnpm install webtoapp-cli@latest", {
            encoding: "utf8",
            timeout: 60000,
            cwd: "/tmp",
          });
    ...
    // Version command test
    await this.runTest("webtoapp-cli Version Command", async () => {
      try {
  ...
  async runE2ETests() {
    ...
    await this.runTest(
      ...
      async () => {
        const pakeDir = path.join(config.PROJECT_ROOT, "src-tauri", ".webtoapp");
        ...
        return new Promise((resolve, reject) => {
          ...
          const checkConfigFiles = () => {
            if (fs.existsSync(pakeDir)) {
              ...
              const pakeConfigFile = path.join(pakeDir, "webtoapp.json");
...
const options = {
  ...
  pakeCliTests: args.includes("--webtoapp-cli"),
...
Optional Components:
  ...
  --webtoapp-cli     Add webtoapp-cli GitHub Actions tests
