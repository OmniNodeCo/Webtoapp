...
describe("Workflow path integration", () => {
  ...
  describe("Workflow artifact uploads", () => {
    it("builds a direct no-installer artifact for every supported OS", () => {
      const workflow = fs.readFileSync(
        ".github/workflows/build.yml",
        "utf8",
      );

      expect(workflow).toContain("Build Windows portable executable");
      expect(workflow).toContain("Build macOS app bundle");
      expect(workflow).toContain("Build Linux AppImage");
      expect(workflow).toContain("--no-bundle");
      expect(workflow).toContain("actions/upload-artifact@v4");
    });

    it("forwards the new-window input on every platform", () => {
      const workflow = fs.readFileSync(
        ".github/workflows/build.yml",
        "utf8",
      );
  ...
  describe("npm package boundaries", () => {
    it("excludes generated Tauri build state from the shipped template", () => {
      ...
      expect(ignoredPaths).toEqual(
        expect.arrayContaining([".webtoapp/", "target/", ".cargo/config.toml"]),
      );
    });
  });
});
