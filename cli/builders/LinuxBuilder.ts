...
export default class LinuxBuilder extends BaseBuilder {
  ...
  getFileName() {
    const { name = 'webtoapp-app', targets } = this.options;
  ...
  private async createArchPackageFromDeb({
    ...
    const workDir = path.resolve('.webtoapp-arch-package');
    ...
    try {
      ...
      // the payload already ships Pake's own com.webtoapp.<name>.desktop.
...
pkgbase = ${packageName}
pkgver = ${version}-1
pkgdesc = ${name} Webtoapp app
url = https://github.com/OmniNodeCo/Webtoapp
...
`,
  ...
  protected getBuildCommand(packageManager: string = 'pnpm'): string {
    const configPath = path.join('src-tauri', '.webtoapp', 'tauri.conf.json');
