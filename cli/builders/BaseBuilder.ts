...
const APPIMAGE_FAILURE_GUIDANCE =
  ...
  'Still stuck? Build a DEB instead: webtoapp <url> --targets deb\n' +
  'Detailed guide: https://github.com/OmniNodeCo/Webtoapp/blob/main/docs/faq.md\n' +
  APPIMAGE_BAR;
...
export default abstract class BaseBuilder {
  ...
  async start(url: string) {
    logger.info('Webtoapp dev server starting...');
  ...
  async buildAndCopy(url: string, target: string, logSuccess = true) {
    const { name = 'webtoapp-app' } = this.options;
    await mergeConfig(url, this.options, tauriConfig);
    ...
    if (isLinuxAppImage && !buildEnv.NO_STRIP && this.options.debug) {
      logger.warn(
        '⚠ AppImage strip step can fail on glibc 2.38+; Webtoapp will auto-retry with NO_STRIP=1.',
      );
    }
  ...
  protected getBinaryName(appName: string): string {
    ...
    return `webtoapp-${nameToUse}${extension}`;
  }
