...
export function resolveLocalAppName(
  ...
  const baseName = path.parse(filePath).name || 'webtoapp-app';
  if (platform === 'linux') {
    return generateLinuxPackageName(baseName) || 'webtoapp-app';
  }
  const normalized = baseName
    ...
    .replace(/\s+/g, ' ')
    .trim();
  return normalized || 'webtoapp-app';
}
...
export default async function handleOptions(
  ...
  const resolvedName = name || 'webtoapp-app';
  ...
  // Webtoapp supports Tauri's direct no-bundle executable on Windows and Linux.
  // macOS keeps an .app bundle because that is its portable native format.
  if (appOptions.bundle === false && platform === 'darwin') {
    logger.warn('✼ --no-bundle is not supported on macOS; building an .app bundle instead.');
