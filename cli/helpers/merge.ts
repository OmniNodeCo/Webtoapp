 ...
 * merged into webtoapp.json. Exposed for snapshot testing so option drift
...
function asSupportedPlatform(platform: NodeJS.Platform): SupportedPlatform {
  if (platform !== 'win32' && platform !== 'darwin' && platform !== 'linux') {
    throw new Error(
      `Webtoapp only supports win32, darwin, and linux; detected '${platform}'.`,
    );
  }
  return platform;
}
...
async function stageLocalTree(sourceDir: string): Promise<void> {
  ...
  if (
    ...
    throw new PakeError(
      `Local input "${sourceDir}" contains the Webtoapp CLI installation itself.`,
      {
        code: 'INVALID_INPUT',
        hint: 'Point Webtoapp at your built output directory, not at a directory containing webtoapp-cli.',
      },
    );
  }
  ...
  try {
    ...
    // every later `webtoapp` invocation would fail until a manual reinstall.
    restoreLocalTree();
    throw error;
  }
}
  ...
  if (stat.isDirectory()) {
    ...
    if (!(await fsExtra.pathExists(path.join(url, entryFile)))) {
      throw new PakeError(
        ...
        {
          code: 'INVALID_INPUT',
          hint: 'Point Webtoapp at the built output directory that contains index.html.',
        },
      );
    }
...
async function mergeLinuxConfig(
  ...
  const desktopFileName = `com.webtoapp.${linuxName}.desktop`;
...
async function writeAllConfigs(
  ...
  const pakeConfigPath = path.join(tauriConfigDirectory, 'webtoapp.json');
...
export async function mergeConfig(
  ...
  const linuxBinaryName = `webtoapp-${generateLinuxPackageName(name)}`;
  tauriConf.mainBinaryName =
    platform === 'linux'
      ? linuxBinaryName
      : `webtoapp-${generateIdentifierSafeName(name)}`;
