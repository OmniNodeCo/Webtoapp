 ...
 * The published schema lives in schema/webtoapp.schema.json.
 */
...
export async function loadConfigFile(
  ...
  if (!(await fsExtra.pathExists(configPath))) {
    throw new PakeError(`Config file not found: ${configPath}`, {
      code: 'INVALID_INPUT',
      hint: 'Pass a path to a JSON file matching schema/webtoapp.schema.json.',
    });
  }
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new PakeError('Config file must contain a JSON object.', {
      code: 'INVALID_INPUT',
      hint: 'See schema/webtoapp.schema.json for the expected shape.',
    });
  }
  ...
  for (const [key, value] of Object.entries(parsed)) {
    ...
    if (!validKeys.has(key)) {
      throw new PakeError(`Unknown config field "${key}".`, {
        code: 'INVALID_INPUT',
        hint: 'Field names are camelCase CLI option names; see schema/webtoapp.schema.json.',
      });
    }
    ...
    if (expected && !matchesType(value, expected)) {
      throw new PakeError(
        ...
        {
          code: 'INVALID_INPUT',
          hint: 'See schema/webtoapp.schema.json for field types.',
        },
      );
    }
    if (typeof value === 'number') {
      ...
      if (
        ...
        throw new PakeError(
          ...
          {
            code: 'INVALID_INPUT',
            hint: 'See schema/webtoapp.schema.json for field ranges.',
          },
        );
      }
    }
