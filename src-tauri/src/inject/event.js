...
function toggleNativeFullscreen(appWindow) {
  appWindow
    ...
    .catch((error) => {
      console.warn("[Webtoapp] Failed to toggle native fullscreen:", error);
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (internalUrlRegex) {
    try {
      ...
      console.error("[Webtoapp] Invalid internal_url_regex pattern:", e);
    }
  }

  if (!document.getElementById("webtoapp-top-dom") && hasImmersiveHeader()) {
    const topDom = document.createElement("div");
    topDom.id = "webtoapp-top-dom";
    document.body.appendChild(topDom);
  }

  const domEl = document.getElementById("webtoapp-top-dom");
  ...
  const isInternalUrl = (url) => {
    ...
    if (internalUrlPattern) {
      try {
        ...
        console.error("[Webtoapp] Error testing internal_url_regex:", e);
  ...
  const detectAnchorElementClick = (e) => {
    ...
    if (anchorElement && anchorElement.href) {
      ...
      if (window.isAuthLink(absoluteUrl)) {
        console.log("[Webtoapp] Handling OAuth navigation in-app:", absoluteUrl);
        e.preventDefault();
