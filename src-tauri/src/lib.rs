...
pub fn run_app() {
    ...
    // - Secondary multi-window labels ("webtoapp-N"): reveal if still hidden (Cmd+N).
    // start_to_tray keeps the main window hidden until the user opens it.
    {
        ...
        app_builder = app_builder.on_page_load(move |webview, payload| {
            ...
            // Multi-window clones (webtoapp-1, webtoapp-2, …) built hidden by
            // open_additional_window_safe.
            if label.starts_with("webtoapp-") {
    ...
    app_builder
        ...
        .unwrap_or_else(|error| {
            eprintln!("[Webtoapp] Fatal error while building Tauri application: {error}");
            std::process::exit(1);
        })
