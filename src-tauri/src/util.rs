...
pub fn get_pake_config() -> (PakeConfig, Config) {
    ...
    let pake_config: PakeConfig = serde_json::from_str(include_str!("../.webtoapp/webtoapp.json"))
        .expect("Failed to parse pake config");

    #[cfg(not(feature = "cli-build"))]
    let pake_config: PakeConfig =
        serde_json::from_str(include_str!("../webtoapp.json")).expect("Failed to parse pake config");

    #[cfg(feature = "cli-build")]
    let tauri_config: Config = serde_json::from_str(include_str!("../.webtoapp/tauri.conf.json"))
...
pub fn show_toast(window: &WebviewWindow, message: &str) {
    ...
    if let Err(error) = window.eval(&script) {
        eprintln!("[Webtoapp] Failed to show toast: {error}");
    }
}
