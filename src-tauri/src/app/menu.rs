...
pub fn set_app_menu(
    ...
    let pake_menu_item_title = format!("Built with Webtoapp V{}", pake_version);
...
fn app_menu(app: &AppHandle<Wry>) -> tauri::Result<Submenu<Wry>> {
    let app_menu = Submenu::new(app, "Webtoapp", true)?;
...
pub fn handle_menu_click(app_handle: &AppHandle, id: &str) {
    match id {
        ...
        "pake_github_link" => {
            let _ = app_handle
                .opener()
                .open_url("https://github.com/OmniNodeCo/Webtoapp", None::<&str>);
        }
        "reload" => {
