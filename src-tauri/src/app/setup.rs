...
pub fn set_system_tray(
    ...
    if !show_system_tray {
        app.remove_tray_by_id("webtoapp-tray");
        return Ok(());
    }
    ...
    app.app_handle().remove_tray_by_id("webtoapp-tray");
    ...
    if let Some(icon) = resolved_icon {
        ...
        eprintln!("[Webtoapp] No tray icon available; tray will build without an icon.");
    }
...
pub fn set_global_shortcut(
    ...
    let shortcut_hotkey = match Shortcut::from_str(&shortcut) {
        Ok(s) => s,
        Err(error) => {
            eprintln!("[Webtoapp] Invalid activation shortcut '{shortcut}': {error}");
            return Ok(());
        }
    };
    ...
    if let Err(error) = app_handle.plugin(
        ...
        eprintln!(
            "[Webtoapp] Failed to register global shortcut plugin '{shortcut}': {error}; continuing without it."
        );
        return Ok(());
    }

    if let Err(error) = app.global_shortcut().register(shortcut_hotkey) {
        eprintln!("[Webtoapp] Failed to bind global shortcut '{shortcut}': {error}");
    }
