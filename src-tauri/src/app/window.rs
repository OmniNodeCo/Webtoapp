...
fn taskbar_icon_handle() -> Option<isize> {
    ...
    *TASKBAR_ICON.get_or_init(|| {
        let executable = match std::env::current_exe() {
            ...
            Err(error) => {
                eprintln!(
                    "[Webtoapp] Failed to resolve the app executable for its taskbar icon: {error}"
                );
                return None;
            }
        };
        ...
        if extracted == 0 || large_icon.is_null() {
            eprintln!(
                "[Webtoapp] Failed to extract the taskbar icon from {}.",
                executable.display()
            );
            return None;
        }
...
pub fn reapply_window_icon(window: &WebviewWindow) {
    if let Some(icon) = window.app_handle().default_window_icon().cloned() {
        if let Err(error) = window.set_icon(icon) {
            eprintln!("[Webtoapp] Failed to re-apply the window icon: {error}");
        }
    }
    ...
    match window.hwnd() {
        ...
        Err(error) => {
            eprintln!("[Webtoapp] Failed to resolve the window handle for its taskbar icon: {error}");
        }
    }
}
...
/// True when any Webtoapp webview window is currently on screen.
///
...
fn build_window_with_label(
    ...
    let window_config = config.windows.first().ok_or_else(|| {
        tauri::Error::Io(std::io::Error::new(
            ...
            "webtoapp.json must define at least one window configuration",
        ))
    })?;
        ...
        "web" => {
            let parsed = window_config.url.parse().map_err(|err| {
                tauri::Error::Io(std::io::Error::new(
                    ...
                    format!(
                        "Invalid 'web' url '{}' in webtoapp.json: {err}",
                        window_config.url
                    ),
                ))
            })?;
    ...
    if window_config.new_window {
        ...
        window_builder = window_builder.on_new_window(move |target_url, features| {
            match open_requested_window(
                ...
                Err(error) => {
                    eprintln!("[Webtoapp] Failed to open requested window: {error}");
                ...
                .on_document_title_changed(|window, title| {
                    if !title.trim().is_empty() {
                        if let Err(error) = window.set_title(&title) {
                            eprintln!("[Webtoapp] Failed to update the macOS tab title: {error}");
                        }
                    }
                });
        }
        ...
        window_builder = window_builder.on_download(move |webview, event| match event {
            DownloadEvent::Requested { url, destination } => {
                match download_handle.path().download_dir() {
                    ...
                    Err(error) => {
                        eprintln!("[Webtoapp] Failed to resolve download dir: {error}");
                    }
                }
                true
            }
        ...
        Queue::main().exec_async(move || match tabbing_window.ns_window() {
            Ok(ns_window_ptr) => unsafe {
                let Some(ns_window) =
                    ...
                    eprintln!("[Webtoapp] Failed to retain the macOS window for tabbing.");
                    return;
                };
                ns_window.setTabbingMode(objc2_app_kit::NSWindowTabbingMode::Preferred);
            },
            Err(error) => {
                eprintln!("[Webtoapp] Failed to access the macOS window for tabbing: {error}");
            }
        });
    }
        ...
        Queue::main().exec_async(move || {
            if let Err(error) = auth_window.with_webview(move |webview| {
                if !crate::app::auth::install_auth_delegate_and_navigate(
                    ...
                    eprintln!("[Webtoapp] Failed to configure macOS authentication handling.");
                }
            }) {
                eprintln!("[Webtoapp] Failed to access the macOS webview: {error}");
            }
        });
    }
