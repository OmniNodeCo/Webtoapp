...
See [PR #744 - Support packaging webtoapp applications into Squirrel-based installers](https://github.com/OmniNodeCo/Webtoapp/pull/744)

## Command Line

```bash
webtoapp [options] [targetUrl] [dest]
```
...
or _[New in 46.0.5]_ the `WEBTOAPP_APPS_DIR` environment variable if set.

**Tip:** Add `export WEBTOAPP_APPS_DIR=~/Applications/` to your
`~/.bashrc` (or `~/.zshrc` or similar) to set the default app destination
if none is passed. This lets you simply run `webtoapp example.com` and
...
Prints the version of your `webtoapp` install.
...
Specifies if the source code within the wrapped app should be packaged into an archive, defaults to false, [read more](https://www.electronjs.org/docs/latest/glossary#asar).
...
-   When packaging for macOS, must be a `.icns` or a `.png` file if the [optional dependencies](../README.md#optional-dependencies) are installed.
    If your `PATH` has our image-conversion dependencies (`iconutil`, and either ImageMagick `convert` + `identify`, or GraphicsMagick `gm`), Webtoapp will automatically convert the `.png` to a `.icns` for you.
...
_[New in 43.1.0]_ This option will attempt to extract all existing options from the old app, and upgrade it using the current Webtoapp CLI.
...
Note: some sites using Widevine (like Udemy or HBO Max) may still refuse to load videos, and require EVS-signing your Webtoapp app to work. Try signing your app using CastLabs tools. See https://github.com/castlabs/electron-releases/wiki/EVS and [#1147](https://github.com/OmniNodeCo/Webtoapp/issues/1147#issuecomment-828750362). TL;DR:
...
```bash
webtoapp <your-website> --browserwindow-options '{ "webPreferences": { "defaultFontFamily": { "standard": "Comic Sans MS", "serif": "Comic Sans MS" } } }'
```
...
```bash
webtoapp <your-geolocation-enabled-website> --process-envs '{"GOOGLE_API_KEY": "<your-google-api-key>"}'
```
...
```sh
webtoapp http://google.com --inject site.css --title-bar-style 'hiddenInset'
```
...
```bash
webtoapp <your-website> --file-download-options '{"saveAs": true}'
```
...
```bash
webtoapp http://google.com --inject ./some-js-injection.js --inject ./some-css-injection.css ~/Desktop
```
...
By default, Webtoapp uses a preset user agent string for your OS and masquerades as a regular Google Chrome browser, so that for some sites, it will not say that the current browser is unsupported.
...
```bash
webtoapp https://google.com --internal-urls ".*?\.google\.*?" --block-external-urls
```
...
Internal URLs will open in Webtoapp, other URLs will open in your preferred browser.
...
```bash
webtoapp https://google.com --internal-urls ".*?\.google\.*?"
```

To turn off base domain matching, use [`--strict-internal-urls`](#strict-internal-urls). Or, if you never expect Webtoapp to open an "external" page in your OS browser,

```bash
webtoapp https://google.com --internal-urls ".*?"
```
...
If you think this list is missing a login page that you think should be internal, feel free to submit an [issue](https://github.com/OmniNodeCo/Webtoapp/issues/new?assignees=&labels=bug&template=bug_report.md&title=[New%20internal%20login%20page%20request]%20Your%20login%20page%20here) or even better a pull request!
...
```bash
webtoapp https://google.com --proxy-rules http://127.0.0.1:1080
```
...
Disables the warning shown when opening a Webtoapp app made a long time ago, using an old and probably insecure Electron. Webtoapp uses the Chrome browser (through Electron), and remaining on an old version is A. performance sub-optimal and B. dangerous.

However, there are legitimate use cases to disable such a warning. For example, if you are using Webtoapp to ship a kiosk app exposing an internal site (over which you have control). Under those circumstances, it is reasonable to disable this warning that you definitely don't want end-users to see.

More description about the options for `webtoapp` can be found at the above [section](#command-line).
...
```bash
webtoapp http://google.com --crash-reporter https://electron-crash-reporter.appspot.com/PROJECT_ID/create/
```
...
```bash
webtoapp <your-geolocation-enabled-website> --win32metadata '{"ProductName": "Your Product Name", "InternalName", "Your Internal Name", "FileDescription": "Your File Description"}'
```
...
_DEPRECATED as of 2021-03-10, will be removed at some point_: There's nothing Webtoapp can do to stop this treadmill, so here it goes.
Flash is triply dead upstream: at Adobe, in Chrome, and now in Electron.
Webtoapp 43.0.0 was just released, and defaults to Electron 12, which
...
You _might_ still be able to use Webtoapp's existing Flash flags while they work,
...
If `--flash` is specified, Webtoapp will automatically try to determine the
...
In addition to CLI flags, Webtoapp offers a programmatic Node.js API.

```bash
# install and save to package.json
npm install --save webtoapp
```

In your `.js` file:

```javascript
var webtoapp = require('webtoapp').default;
...
webtoapp(options, function (error, appPath) {
    if (error) {
        console.error(error);
        return;
    }
    console.log('App has been wrapped to', appPath);
});
```

## Accessing the Electron session

Generated Webtoapp windows use context isolation and keep Node integration
disabled. The `--browserwindow-options` flag may customize ordinary
BrowserWindow settings, but Webtoapp intentionally ignores attempts to replace
the preload, enable Node integration, disable context isolation, or disable
web security.

This is deliberate: a wrapped page can contain third-party scripts, and an
XSS must not automatically become local code execution. If a site needs a new
Electron capability, add a narrowly scoped, reviewed Webtoapp option or bridge
rather than exposing `ipcRenderer` to every page. JavaScript passed to
`--inject` runs in the page's main world and cannot call Node APIs directly.

The generated runtime exposes only the small `window.webtoappBridge` surface
needed for desktop notifications and screen-source requests. Treat injected
files as application code and review them before packaging.
