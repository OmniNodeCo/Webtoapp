...
export default {
  input: isProduction ? "cli/cli.ts" : "cli/dev.ts",
  output: {
  ...
  external: (id) => {
    if (id === "cli/cli.ts" || id === "cli/dev.ts") return false;
  ...
  plugins: [
    ...
    alias({
      entries: [{ find: "@", replacement: path.join(appRootPath.path, "cli") }],
    }),
    ...devPlugins,
  ],
};
  ...
  return {
    ...
    buildEnd() {
      ...
      cliChildProcess.on("close", async (code) => {
        ...
        const command = `${packageManager} run tauri dev --config ./src-tauri/.webtoapp/tauri.conf.json -- --features cli-build`;
