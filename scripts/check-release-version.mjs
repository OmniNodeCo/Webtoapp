...
function extractCargoLockVersion() {
  ...
  const match = cargoLock.match(
    /\[\[package\]\]\s+name = "webtoapp"\s+version = "([^"]+)"/,
  );
  return match?.[1];
}
...
expectEqual(
  ...
  "git+https://github.com/OmniNodeCo/Webtoapp.git",
);
...
if (!packageJson.files?.includes("NOTICE.md")) {
  errors.push(
    "package.json files: NOTICE.md must be included in the npm package",
  );
}
