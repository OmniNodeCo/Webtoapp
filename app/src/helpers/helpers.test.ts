...
test('all urls should be internal with wildcard regex', () => {
    expect(
        linkIsInternal(internalUrl, externalUrl, wildcardRegex, true),
    ).toEqual(true);
});

test('a "www." of a domain should be considered internal', () => {
    expect(
        linkIsInternal(internalUrl, internalUrlWww, undefined, false),
    ).toEqual(true);
});
...
const testNonLoginPages = [
    ...
    'https://github.com/OmniNodeCo/Webtoapp',
    'https://github.com/org/webtoapp',
    'https://microsoft.com',
...
test.each(testNonLoginPages)(
    ...
    (url: string) => {
        expect(linkIsInternal(internalUrl, url, undefined, false)).toEqual(
            false,
        );
    },
);
