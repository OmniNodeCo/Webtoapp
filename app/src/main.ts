...
// Entrypoint for Squirrel, a windows update framework. See https://github.com/OmniNodeCo/Webtoapp/pull/744
/* Squirrel startup integration is intentionally disabled until it is tested
 * against the generated Windows package.
...
// Webtoapp is a browser, and an old browser is an insecure / badly-performant one.
...
if (appArgs.processEnvs) {
    ...
    if (typeof appArgs.processEnvs === 'string') {
        try {
            processEnvs = JSON.parse(appArgs.processEnvs) as Record<
                string,
                string
            >;
        } catch {
...
async function onReady(): Promise<void> {
    ...
    if (appArgs.globalShortcuts) {
        ...
        if (isOSX() && appArgs.accessibilityPrompt) {
            ...
            const globalShortcutsKeys = appArgs.globalShortcuts.map(
                (g) => g.key,
            );
    ...
    if (
        ...
        const oldBuildWarningText =
            ...
            'This app was built a long time ago. Webtoapp uses the Chrome browser (through Electron), and it is insecure to keep using an old version of it. Please upgrade Webtoapp and rebuild this app.';
        dialog
