...
async function copyIconsIfNecessary(
    ...
    if (
        ...
        if (options.webtoapp.tray !== 'false') {
...
function trimUnprocessableOptions(options: AppOptions): void {
    if (options.packager.platform === 'win32' && !isWindows() && !hasWine()) {
        const packagerOptions = options.packager as unknown as Record<
            string,
            unknown
        >;
        const optionsPresent = OPTIONS_REQUIRING_WINDOWS_FOR_WINDOWS_BUILD.filter(
            (key) => Boolean(packagerOptions[key]),
        );
        if (optionsPresent.length === 0) {
            return;
        }
        log.warn(
            `*Not* setting [${optionsPresent.join(
                ', ',
            )}], as couldn't find Wine.`,
        ...
        for (const keyToUnset of optionsPresent) {
            packagerOptions[keyToUnset] = undefined;
        }
    }
}
...
function getOSRunHelp(platform?: string): string {
    if (platform === 'win32') {
        ...
        return `the contained executable file (prefixing with ./ if necessary)\nMenu/desktop shortcuts are up to you, because Webtoapp cannot know where you're going to move the app. Search for "linux .desktop file" for help, or see https://wiki.archlinux.org/index.php/Desktop_entries`;
...
export async function buildWebtoappApp(
    ...
    log.info(
        '\n\n    Webtoapp packages websites as desktop apps with Electron.\n' +
            '    The generated app is independent of the Webtoapp CLI after it is built.\n\n',
    );
    ...
    if (isUpgrade(rawOptions)) {
        ...
        if (!oldApp) {
            throw new Error(
                `Could not find an existing Webtoapp app in "${
    ...
    const appPathArray = await electronPackager(
        options.packager as unknown as Parameters<typeof electronPackager>[0],
    );
