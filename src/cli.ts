...
import { buildWebtoappApp } from './main';
...
export function initArgs(argv: string[]): yargs.Argv<RawOptions> {
    ...
    const args = yargs(sanitizedArgs)
        ...
        .example(
            ...
            'Upgrade (in place) the existing Webtoapp app at <pathToExistingApp>',
        )
        .example(
        ...
        .example(
            ...
            'See https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md',
        )
        ...
        .positional('outputDirectory', {
            defaultDescription:
                'defaults to the current directory, or env. var. WEBTOAPP_APPS_DIR if set',
        ...
        .option('global-shortcuts', {
            description:
                'define global keyboard shortcuts via a JSON file; See https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#global-shortcuts',
            normalize: true,
            type: 'string',
        })
        .option('i', {
        ...
        .option('no-overwrite', {
            default: false,
            description:
                'do not overwrite output directory if it already exists',
            type: 'boolean',
        })
        .option('overwrite', {
        ...
        .option('portable', {
            default: false,
            description:
                'make the app store its user data in the app folder; WARNING: see https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#portable for security risks',
            type: 'boolean',
        })
        .option('upgrade', {
            description:
                'upgrade an app built by an older version of Webtoapp\nYou must pass the full path to the existing app executable (app will be overwritten with upgraded version by default)',
            normalize: true,
            type: 'string',
        })
        .option('widevine', {
        ...
        .option('bookmarks-menu', {
            description:
                'create a bookmarks menu (via JSON file); See https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#bookmarks-menu',
            normalize: true,
            type: 'string',
        })
        .option('browserwindow-options', {
            coerce: parseJson,
            description:
                'override Electron BrowserWindow options (via JSON string); see https://github.com/OmniNodeCo/Webtoapp/blob/main/DOCS.md#browserwindow-options',
        })
        ...
        .option('disable-old-build-warning-yesiknowitisinsecure', {
            default: false,
            description:
                'disable warning shown when opening an app made too long ago; Webtoapp uses the Chrome browser (through Electron), and it is dangerous to keep using an old version of it',
            type: 'boolean',
        })
...
export function parseArgs(args: yargs.Argv<RawOptions>): RawOptions {
    ...
    if (parsed.upgrade && parsed.targetUrl) {
        ...
        if (targetAndUpgrade) {
            throw new Error(
                'ERROR: Webtoapp must be called with either a targetUrl or the --upgrade option, not both.\n',
            );
        }
    }
    ...
    if (!parsed.targetUrl && !parsed.upgrade) {
        throw new Error(
            'ERROR: Webtoapp must be called with either a targetUrl or the --upgrade option.\n',
        );
    }
    ...
    for (const arg of [
        ...
        if (parsed[arg] && typeof parsed[arg] === 'string') {
            ...
            // as parsed object as they were still strings in `webtoapp.json`
...
function sanitizeArgs(argv: string[]): string[] {
    ...
    argv.forEach((arg) => {
        if (isArgFormatInvalid(arg)) {
            throw new Error(
                `Invalid argument passed: ${arg} .\nWebtoapp supports short options (like "-n") and long options (like "--name"), all lowercase. Run "webtoapp --help" for help.\nAborting`,
            );
        }
...
if (require.main === module) {
    ...
    if (!options.out && process.env.WEBTOAPP_APPS_DIR) {
        options.out = process.env.WEBTOAPP_APPS_DIR;
    }

    buildWebtoappApp(options).catch((error) => {
