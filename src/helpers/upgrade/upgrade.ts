...
function findUpgradeAppResourcesDir(searchDir: string): string | null {
    ...
    log.debug(`Searching for webtoapp.json in ${searchDir}`);
    const children = fs.readdirSync(searchDir, { withFileTypes: true });
    if (fileExists(path.join(searchDir, 'webtoapp.json'))) {
        // Found 'webtoapp.json', so this must be it!
    ...
    for (const childDir of childDirectories) {
        ...
        const result = findUpgradeAppResourcesDir(
            path.join(searchDir, childDir.name, 'webtoapp.json'),
        );
        if (result !== null) {
...
export function findUpgradeApp(upgradeFrom: string): UpgradeAppInfo | null {
    ...
    if (appResourcesDir === null) {
        log.debug(`No webtoapp.json file found in ${searchDir}`);
        return null;
    }

    const webtoappJSONPath = path.join(appResourcesDir, 'webtoapp.json');

    log.debug(`Loading ${webtoappJSONPath}`);
    let options = parseJson<WebtoappOptions>(
        fs.readFileSync(webtoappJSONPath, 'utf8'),
    );

    if (!options) {
        throw new Error(
            `Could not read Webtoapp options from ${webtoappJSONPath}`,
        );
    }
    ...
    return {
        ...
        options: {
            ...
            asar:
                options.asar !== undefined
                    ? options.asar
                    : isAsar(appResourcesDir),
