...
import { WebtoappOptions } from '../../../shared/src/options/model';
...
export function getOptionsFromExecutable(
    ...
    const newOptions: WebtoappOptions = { ...priorOptions };
    if (!newOptions.name) {
        throw new Error(
    ...
    const looksLikeMacOS =
        children.filter((c) => c.name === 'MacOS' && c.isDirectory()).length >
        0;
    const looksLikeWindows =
        children.filter(
            (c) => c.name.toLowerCase().endsWith('.exe') && c.isFile(),
        ).length > 0;
    const looksLikeLinux =
        children.filter(
            (c) => c.name.toLowerCase().endsWith('.so') && c.isFile(),
        ).length > 0;
    ...
    if (looksLikeMacOS) {
        ...
        if (newOptions.platform === undefined) {
            newOptions.platform =
                children.filter((c) => c.name === 'Library' && c.isDirectory())
                    .length > 0
                    ? 'mas'
                    : 'darwin';
        }
        ...
        executablePath = path.join(
            ...
            children.filter(
                (c) =>
                    c.name.toLowerCase() === `${name.toLowerCase()}.exe` &&
                    c.isFile(),
            )[0].name,
        );
        ...
        if (newOptions.buildVersion === undefined) {
            ...
            newOptions.buildVersion = getVersionString(
                executablePath,
                'FileVersion',
            );
    ...
    if (newOptions.arch === undefined) {
        ...
        log.debug(
            `Extracted arch from executable: ${newOptions.arch as string}`,
        );
    }
