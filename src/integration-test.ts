...
async function checkApp(
    ...
    const configPath = path.join(appPath, 'webtoapp.json');
    const webtoappConfig: WebtoappOptions | undefined =
        parseJson<WebtoappOptions>(fs.readFileSync(configPath).toString());
    expect(webtoappConfig).not.toBeUndefined();

    expect(inputOptions.targetUrl).toBe(webtoappConfig?.targetUrl);

    // Test name inferring
    expect(webtoappConfig?.name).toBe('npm');
    ...
    if (inputOptions.arch !== undefined) {
        expect(inputOptions.arch).toEqual(webtoappConfig?.arch);
    } else {
        expect(os.arch()).toEqual(webtoappConfig?.arch);
    }

    // Test electron version
    expect(webtoappConfig?.electronVersionUsed).toBe(
    ...
    if (inputOptions.userAgent) {
        const translatedUserAgent = await userAgent({
            ...
            webtoapp: { userAgent: inputOptions.userAgent },
        });
        inputOptions.userAgent = translatedUserAgent || inputOptions.userAgent;
    }

    expect(webtoappConfig?.userAgent).toEqual(inputOptions.userAgent);

    // Test lang
    expect(webtoappConfig?.lang).toEqual(inputOptions.lang);
    ...
    if (inputOptions.globalShortcuts) {
        ...
        expect(webtoappConfig?.globalShortcuts).toStrictEqual(shortcutData);
    }
}
...
describe('Webtoapp', () => {
    ...
    test.each(['darwin', 'linux'])(
        'builds a Webtoapp app for platform %s',
        async (platform) => {
            ...
            const appPath = await buildWebtoappApp(options);
...
describe('Webtoapp upgrade', () => {
    ...
    test.each([
        ...
        'can upgrade a Webtoapp app for platform/arch: %s',
        async (baseAppOptions) => {
            ...
            const upgradeAppPath = await buildWebtoappApp(upgradeOptions);
...
describe('Browser version retrieval', () => {
    test('get chrome version with electron version', async () => {
        await expect(
            getChromeVersionForElectronVersion('12.0.0'),
        ).resolves.toBe('89.0.4389.69');
    });
