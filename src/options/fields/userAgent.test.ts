...
test('when a userAgent parameter is passed', async () => {
    const params = {
        packager: {},
        webtoapp: { userAgent: 'valid user agent' },
    };
...
test('edge userAgent parameter is passed', async () => {
    ...
    const params = {
        ...
        webtoapp: { userAgent: 'edge' },
    };

    const parsedUserAgent = await userAgent(params);

    expect(parsedUserAgent).not.toBe(params.webtoapp.userAgent);
