...
export async function userAgent(
    ...
    if (
        !Object.keys(USER_AGENT_SHORT_CODE_MAPS).includes(
            options.webtoapp.userAgent.toLowerCase(),
        )
    ) {
        // Real user agent got passed. No need to translate it.
        log.debug(
            `${options.webtoapp.userAgent.toLowerCase()} not found in`,
    ...
    const mapFunction =
        USER_AGENT_SHORT_CODE_MAPS[options.webtoapp.userAgent];
