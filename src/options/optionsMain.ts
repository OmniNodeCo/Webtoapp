...
const SEMVER_VERSION_NUMBER_REGEX = /^\d+\.\d+\.\d+(?:[-+_][\w.-]+)?$/;

/**
 * Process and validate raw user arguments
 */
function validateWindowNumber(
    ...
    if (
        ...
        throw new Error(
            `Invalid --${optionName}: expected a finite number greater than or equal to ${minimum}`,
        );
    }
}
...
export async function getOptions(rawOptions: RawOptions): Promise<AppOptions> {
    validateWindowNumber(rawOptions.width, 'width', 1);
    validateWindowNumber(rawOptions.height, 'height', 1);
    validateWindowNumber(rawOptions.minWidth, 'min-width', 0);
    validateWindowNumber(rawOptions.minHeight, 'min-height', 0);
    validateWindowNumber(rawOptions.maxWidth, 'max-width', 1);
    validateWindowNumber(rawOptions.maxHeight, 'max-height', 1);
    if (rawOptions.zoom !== undefined && rawOptions.zoom <= 0) {
        throw new Error('Invalid --zoom: expected a number greater than 0');
    }
    ...
    const options: AppOptions = {
        packager: {
            ...
            electronVersion:
                rawOptions.electronVersion ?? DEFAULT_ELECTRON_VERSION,
        ...
        webtoapp: {
            ...
            webtoappVersion: packageJson.version,
            quiet: rawOptions.quiet ?? false,
    ...
    } else if (options.webtoapp.quiet) {
        log.setLevel('silent');
    } else {
        log.setLevel('info');
    }
    ...
    if (options.webtoapp.flashPluginDir) {
        options.webtoapp.insecure = true;
    }

    if (options.webtoapp.userAgentHonest && options.webtoapp.userAgent) {
        options.webtoapp.userAgent = undefined;
        log.warn(
    ...
    if (
        options.webtoapp.maxWidth &&
        options.webtoapp.width &&
        options.webtoapp.width > options.webtoapp.maxWidth
    ) {
        options.webtoapp.width = options.webtoapp.maxWidth;
    }

    if (
        options.webtoapp.maxHeight &&
        options.webtoapp.height &&
        options.webtoapp.height > options.webtoapp.maxHeight
    ) {
        options.webtoapp.height = options.webtoapp.maxHeight;
    }
    ...
    if (rawOptions.globalShortcuts) {
        if (typeof rawOptions.globalShortcuts === 'string') {
            ...
            log.debug(
                'Using global shortcuts file at',
            ...
            options.webtoapp.globalShortcuts = globalShortcuts;
        } else {
            // This is an object we got from an existing config in an upgrade
            log.debug(
                'Using global shortcuts object',
                rawOptions.globalShortcuts,
            );
            options.webtoapp.globalShortcuts = rawOptions.globalShortcuts;
        }
    }
